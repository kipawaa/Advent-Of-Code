...
def grade_hand(handbid):
    hand = handbid[0]

    hand = ''.join([i if i in '23456789' else value_dict[i] for i in hand])

    # if there is one distinct card value
    if len(set(hand)) == 1:
        return '7' + hand

    # if there are two distinct card values
    if len(set(hand)) == 2:
        # if the first card occurs once or four times then four of a kind
        num = hand.count(hand[0])
        if num == 1 or num == 4:
            return '6' + hand

        # if not four of a kind then it must be a full house
        return '5' + hand

    # if there are three distinct card values
    if len(set(hand)) == 3:
        for card in hand:
    ...
    # if there are four distinct card values then it must be a pair
    if len(set(hand)) == 4:
        return '2' + hand

    # if there are four distinct card values it must be a high card
    return '1' + hand


def get_total_winnings(game):
    ranks = sorted(game, key=grade_hand)
